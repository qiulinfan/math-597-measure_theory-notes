...
\section{Measurability of limit loci.}
  Let $(X, \cA)$ be a measurable space. For each $n\in \bN$, let $f_n\colon X\to \mathbb{R}$ be a measurable function. Consider the set
  \[
  ...
  \end{itemize}
  \textit{Hint}: note that $\pm\infty$ are not real numbers, and please avoid considering $\infty-\infty$; you may want to prove a lemma to the effect that if $g,h\colon X\to\bar\R$ are measurable functions, then the set
  \[
    \{x\in X\mid g(x)=h(x)\in\R\}
  \]
...
\]
Since each $f_n$ is measurable function, by proposition in lecture (sequential preservation of measurability), $g,h$ are measurable.
...
For each \(\kappa \in \mathbb{N}\), we define:
\[
g_\kappa(x):=\min\{\max\{g(x), -\kappa\}, \kappa\} \quad \text{and} \quad h_\kappa(x):=\min\{\max\{h(x), -\kappa\}, \kappa\}.
\]

\textbf{Claim 1.1: \[
g(x)=h(x)\in\mathbb{R}\quad\Longleftrightarrow\quad \exists \kappa_0>0,\ \forall \kappa\ge\kappa_0,\quad g_\kappa(x)=h_\kappa(x).
\]}
\textbf{proof of claim 1.1:}


1. **充分性**  
   如果 \(g(x)\) 和 \(h(x)\) 都是有限数且相等，则设
   \[
   M=\max\{|g(x)|, |h(x)|\} < \infty.
   \]
   取任何 \(\kappa > M\)，显然有
   \[
   g_\kappa(x)=g(x) \quad \text{和} \quad h_\kappa(x)=h(x).
   \]
   故 \(g_\kappa(x)=h_\kappa(x)\)。而由于 \(M\)固定，对于所有 \(\kappa>M\) 都有 \(g_\kappa(x)=h_\kappa(x)\)。

2. **必要性**  
   反过来，若存在一个 \(x\) 满足：对于所有足够大的 \(\kappa\)（即当 \(\kappa\) 超过某个阈值后），都有 \(g_\kappa(x)=h_\kappa(x)\)。  
   记住，对于任一 \(x\)，当 \(\kappa\) 超过 \(|g(x)|\) 或 \(|h(x)|\) 时，截断函数就“稳定”了，即
   \[
   g_\kappa(x)=g(x) \quad \text{和} \quad h_\kappa(x)=h(x).
   \]
   因此必须有 \(g(x)=h(x)\)（且必然是有限的）。
...
\end{proof}
  ...
  \section{Measurability of continuity loci.}
    Let $(X,d)$ be a metric space, and $f\colon X\to\bC$ any function. Prove that the set of points $x\in X$ such that $f$ is continuous at $x$ is a $G_\delta$-set, and in particular a Borel set.
...
\section{Measurability of differentiability loci.}
  Let $f\colon\R\to\R$ be any function. Let us say (as usual) that $f$ is \emph{differentiable} at $x$ if there exists $\lambda\in\R$ such that $\lim_{y\to x}\frac{f(y)-f(x)}{y-x}=\lambda$. We also declare $f$ to be \emph{strongly differentiable} at $x$ if there exists $\lambda\in\R$ with the following property: for each $\epsilon>0$ there exists $\delta>0$ such that if $|y-x|\le\delta$ and $|z-x|\le\delta$, then $|f(y)-f(z)-\lambda(y-z)|\le\e|y-z|$.
  \begin{itemize}
  \item[(a)]
    Does $f$ being differentiable at $x$ imply that $f$ is strongly differentiable at $x$? Give a proof or a counterexample.
  \item[(b)]
    Prove that the set of points $x\in\R$ at which $f$ is strongly differentiable is a Borel set. \textit{Hint}: consider sets of the form
    \[
      E_{\lambda,m,n}:=\{x\in\R\mid |f(y)-f(z)-\lambda(y-z)|\le\tfrac1n|y-z|\
      \text{whenever $\max\{|y-x|,|z-x|\le\tfrac1m$}\}.
      \]
  \item[(c)]
    \textit{Extra credit}: is the set of points $x\in\R$ at which $f$ is differentiable a Borel set?
  \end{itemize}
...
\section{Monotone convergence?}
  ...
  \begin{itemize}
  \item[(a)]
    Prove that if $\lim_n\int f_n<\infty$, then $\lim_n\int f_n=\int\lim_nf_n$. 
  \item[(b)]
    Give an example of a decreasing sequence $(f_n)_n$ of nonnegative measurable functions such that $\lim_n\int f_n\ne\int\lim_nf_n$. 
  \end{itemize}
