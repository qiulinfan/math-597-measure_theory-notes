...
\]
We use $I_n := \bigl(x_{n},\,x_{n} + \delta_{n}\bigr)$ to denote each component interval, and $\delta_n := \frac{\alpha}{n(n-1)}$ to denote the length of each interval. \\
Let $r>0$ (WLOG $r<1$), then we have  \[
     \frac{1}{n+1} < r \;\le\; \frac{1}{n} \quad \text{ for some } n \in \mathbb{N}
   \]
Then for every \(k \ge n+2\), we have \(\frac{1}{k} < \frac{1}{n+1} < r\). Hence $I_k$ is \textbf{entirely contained} in \((0,r)\). \\
   Conversely, for \(k \le n+1\), we have \(\frac{1}{k} \ge \frac{1}{n+1}\), so \(I_k\) does not lie entirely to the left of \(r\). Therefore: \[
     E \cap (0,r)  = \bigcup_{k=n+2}^\infty I_k   \subseteq (0,r) \]  The measure of the portion of \(E\) in \((0,r)\) is then \[
     \lambda\bigl(E \cap (0,r)\bigr) = 
     \sum_{k = n+2}^\infty \delta_k = 
...
Therefore  \[
    m\bigl(E \cap (-r,r)\bigr) = 
     ...
     \frac{1}{n+1} < r \le \frac{1}{n}
   \]
Since \(n\) can be taken arbitrarily large as \(r \to 0^+\), we conclude \[
     \lim_{r\to 0^+}  \frac{m\bigl(E \cap (-r,r)\bigr)}{m((-r,r))} =    
     \lim_{r\to 0^+} 
     \frac{m\bigl(E \cap (-r,r)\bigr)}{2r} = 
     \frac{\alpha}{2}
   \]
Hence, \(E\) indeed has Lebesgue density \(\alpha/2\) at \(0\).
\end{proof}
